...
#cpsam - count of spam message
#cham - count of ham message
cspam = xf['Type'].value_counts()['spam']
cham = xf['Type'].value_counts()['ham']
...
for word in dictionary:
   spam_list[word] = (xf.loc[xf['Type'] == 'spam', word].sum() + alpha) / (cspam + 2*alpha)
   ham_list[word] = (xf.loc[xf['Type'] == 'ham', word].sum() + alpha) / (cham + 2*alpha)
...
for message , mtype in zip(test_x, test_y):
   if(classify(message) == 'spam' and mtype =='spam'):
    ...
    count+=1
total = len(test_y)
print("Accuracy using Multivariate Distribution is " + str((count/total)*100))